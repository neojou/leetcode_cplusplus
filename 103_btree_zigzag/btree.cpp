...
class Solution {
private:
     vector<vector<int>> &zigzagLevelOrder1(vector<vector<int>> &vvi, TreeNode* root, int level, bool zag) {
        if (root == nullptr)
            return vvi;
        ...
        if (level >= vvi.size()) {
            ...
            if (zag)
                vi.insert(vi.begin(), root->val);
            else
                vi.push_back(root->val);
        }
        
        if (root->left)
            zigzagLevelOrder1(vvi, root->left, level + 1, !zag);
        
        if (root->right)
            zigzagLevelOrder1(vvi, root->right, level + 1, !zag);
...
public:
    vector<vector<int>> zigzagLevelOrder(TreeNode* root) {
        ...
        if (root->left)
            zigzagLevelOrder1(vvi, root->left, 1, true);
        
        if (root->right)
            zigzagLevelOrder1(vvi, root->right, 1, true);
...
int main() {
    ...
    vector<vector<int>> output = s.zigzagLevelOrder(root);
    cout << "Output: ";
